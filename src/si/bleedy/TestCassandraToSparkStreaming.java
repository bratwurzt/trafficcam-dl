...
//        new CassandraReceiver(StorageLevel.MEMORY_ONLY(), CassandraStreamingJavaUtil.javaFunctions(ssc), "17.12.2015 20:28", "16.12.2015 04:01", batchDuration)
//    );
    ArrayBlockingQueue<JavaRDD<ObservationData>> queue = new ArrayBlockingQueue<JavaRDD<ObservationData>>(50);
    JavaInputDStream<ObservationData> cassStream = ssc.queueStream(queue, true);

    cassStream.foreachRDD(new Function<JavaRDD<ObservationData>, Void>()
    {
      @Override
    ...
    long start;
    long fromTime = dateFormat.parse("10.12.2015 20:28").getTime();
    long toTime = dateFormat.parse("21.12.2015 04:01").getTime();
    while (fromTime != toTime)
    {
      start = System.currentTimeMillis();
      long to = fromTime + batchDuration.milliseconds();
      if (to > toTime)
      {
        to = toTime;
      }
      JavaRDD<ObservationData> map = CassandraStreamingJavaUtil.javaFunctions(ssc)
      ...
      while (System.currentTimeMillis() - start < batchDuration.milliseconds())
      {
        Thread.sleep(batchDuration.milliseconds() / 100);
      }
      queue.put(map);
      fromTime = to;
    }
