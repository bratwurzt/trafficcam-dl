...
public abstract class SaveCounterToDbRunnable implements Runnable
{
  ...
  protected abstract void saveToDb(Long counterId, DateTime timestamp, int speed, int carsPerHour, float avgSecGap, double xCoordinates, double yCoordinates) throws SQLException;
  ...
  private void processJson(JsonObject object) throws SQLException
  {
    ...
    for (JsonValue value : object.getJsonArray("Contents").getJsonObject(0).getJsonObject("Data").getJsonArray("Items"))
    {
      ...
      for (final JsonValue data : st.getJsonArray("Data"))
      {
        ...
        final Long counterId = counterMap.get(identity);
        if (counterId == null)
        {
          insertNewCounter(identity, xCoordinates, yCoordinates);
        }
        saveToDb(counterId, modifiedTime, speed, carsPerHour, avgSecGap, xCoordinates, yCoordinates);
      }
    }
    executeBatch();
  }

  abstract void insertNewCounter(String identity, double xCoordinates, double yCoordinates) throws SQLException;
